...
const AnimeForm = ({ anime, heading }: AnimeFormProps) => {
    ...
    const onSubmit = useCallback((values: z.infer<typeof animeFormSchema>) => {
        ...
        <>
            <Form {...form} >
                <form onSubmit={form.handleSubmit(onSubmit)} className="mx-auto mb-16 max-w-[380px] space-y-3 sm:mb-0 sm:max-w-[1000px]">
                    ...
                    <Button type="submit"
                        disabled={form.formState.isDirty || form.formState.isLoading || form.formState.isSubmitting || form.formState.isValidating || userData?.role === 'USER'} className="block w-full disabled:cursor-not-allowed disabled:opacity-60 sm:w-max">
                        Submit
                    </Button>
                </form>
            </Form>
